...
void Reset_Handler(void)
{
	for(uint32_t Local_u32Iterator  = 0; Local_u32Iterator < (uint32_t)&_edata - (uint32_t)&_sdata ;Local_u32Iterator++)// copying data section
	{
		((uint32_t*)&_sdata)[Local_u32Iterator] = ((uint32_t*)&_erodata)[Local_u32Iterator];
	}
	
	for(uint32_t Local_u32Iterator  = 0 ; Local_u32Iterator < (uint32_t)&_ebss - (uint32_t)&_sbss ;Local_u32Iterator++)// initialize .bss section
		((uint32_t*)&_sbss)[Local_u32Iterator] = 0;
